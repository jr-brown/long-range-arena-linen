...
def build_vocab(train_dataset, fields, vocab_size=None, min_freq=0):
    ...
    logging.info("Vocab size: before filtering (%d), after(%d)", len(word_freqs), len(vocab))
...
# -----------------------------------------------------------------------------
def dump_chars_to_textfile(dataset, maxchars=1e9, data_keys=("inputs", "targets")):
    ...
    with tempfile.NamedTemporaryFile(delete=False, prefix="/tmp/ds_chars") as outfp:
        while char_count < maxchars:
            ...
            for k in data_keys:
                ...
                char_count += len(line)
                outfp.write(line)
    logging.info("%d examples processed for training sentencepiece tokenizer.", processed_examples)
...
def train_sentencepiece(dataset,
    ...
    """
    fname, _ = dump_chars_to_textfile(dataset, maxchars=maxchars, data_keys=data_keys)
    ...
    if jax.process_index() == 0:
        ...
        tf.io.gfile.copy(model_fp.name + ".vocab", copy_rename_path + ".vocab", overwrite=True)
        tf.io.gfile.rename(copy_rename_path + ".vocab", model_path + ".vocab", overwrite=True)
...
def load_sentencepiece_tokenizer(model_path,
    ...
    sp_tokenizer = tftxt.SentencepieceTokenizer(model=sp_model, add_bos=add_bos, add_eos=add_eos,
                                                reverse=reverse)
    return sp_tokenizer
...
def load_tfds_dataset(data_dir, dataset_name, split, shuffle=True):
    ...
    dataset = tfds.load(name=dataset_name, split=split, data_dir=data_dir, shuffle_files=shuffle)
    keys = None
