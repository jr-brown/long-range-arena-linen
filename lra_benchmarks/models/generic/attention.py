...
class GenericAttention(nn.Module):
    ...
    max_len: int=512
    layer_num: int=0
    attention_fn_module: Callable[..., Array]=MaskedDotProductAttention
    padded_length_fn: Optional[Callable[[nn.Module], int]]=None
    attention_fn_module_kwargs: Optional[dict[str, Any]]=None
    ...
    def __call__(self, inputs_q, inputs_kv, *, segmentation=None, key_segmentation=None,
                 ...
                 deterministic: bool=False, attention_fn_kwargs: Optional[dict[str, Any]]=None):
        ...
        if self.attention_fn_module_kwargs is None:
            attention_fn_module_kwargs = {}
        else:
            attention_fn_module_kwargs = self.attention_fn_module_kwargs

        if attention_fn_kwargs is None:
            attention_fn_kwargs = {}

        # apply attention
        x = self.attention_fn_module(
