...
gin.external_configurable(lambda x: jax.nn.gelu(x, approximate=False), 'jgelu')  # Needs to be exact, although might be slower. See https://github.com/google/jax/issues/4428.
...
def nonnegative_softmax_kernel_feature_creator(data, projection_matrix, attention_dims_t,
                                               batch_dims_t, precision, is_query,
                                               normalize_data=True, eps=0.0001):
    ...
    if is_query:
        data_dash = ratio * (jnp.exp(data_dash - diag_data - jnp.max(data_dash, axis=last_dims_t,
                                                                     keepdims=True)) + eps)
    else:
        data_dash = ratio * (
            jnp.exp(data_dash - diag_data - jnp.max(data_dash, axis=last_dims_t + attention_dims_t,
                                                    keepdims=True)) + eps)

    return data_dash


def sincos_softmax_kernel_feature_creator(data, projection_matrix, attention_dims_t, batch_dims_t,
                                          precision, normalize_data=True):
...
def generalized_kernel_feature_creator(data, projection_matrix, batch_dims_t, precision, kernel_fn,
                                       kernel_epsilon, normalize_data):
...
def make_fast_softmax_attention(qkv_dim,
    ...
    if ortho_features:
        ...
        matrix_creator = partial(GaussianUnstructuredRandomMatrix, nb_features, qkv_dim)

    if nonnegative_features:
        def kernel_feature_creator(data, projection_matrix, attention_dims_t, batch_dims_t,
                                   precision, is_query, normalize_data=True):
        ...
        def kernel_feature_creator(data, projection_matrix, attention_dims_t, batch_dims_t,
                                   precision, is_query, normalize_data=True):
            del is_query
            return sincos_softmax_kernel_feature_creator(data, projection_matrix, attention_dims_t,
                                                         batch_dims_t, precision, normalize_data)
...
def make_fast_generalized_attention(qkv_dim,
    ...
    if features_type == 'ortho':
        matrix_creator = partial(GaussianOrthogonalRandomMatrix, nb_features, qkv_dim,
                                 scaling=False)
    ...
    def kernel_feature_creator(data, projection_matrix, attention_dims_t, batch_dims_t, precision,
                               is_query, normalize_data=False):
        del attention_dims_t
        del is_query
        return generalized_kernel_feature_creator(data, projection_matrix, batch_dims_t, precision,
                                                  kernel_fn, kernel_epsilon, normalize_data)
...
def _numerator(z_slice_shape, precision, unroll=1):
    ...
    def bwd(pqkv, W_ct):
        ...
        p, qs, ks, vs = pqkv
        _, (qs_ct, ks_ct, vs_ct) = lax.scan(body, (p, jnp.zeros_like(p)), (qs, ks, vs, W_ct),
...
def _denominator(t_slice_shape, precision, unroll=1):
    ...
    def bwd(qkp, R_ct):
        ...
        qs, ks, p = qkp
        _, (qs_ct, ks_ct) = lax.scan(body, (p, jnp.zeros_like(p)), (qs, ks, R_ct), reverse=True,
                                     unroll=unroll)
        return (qs_ct, ks_ct)
...
class FastAttentionviaLowRankDecomposition(FastAttention):
    ...
    # For optimal GPU performance, set lax_scan_unroll to 16.
    def __init__(self, matrix_creator, kernel_feature_creator, renormalize_attention,
                 numerical_stabilizer, redraw_features, unidirectional, lax_scan_unroll=1):
    ...
    def dot_product_attention(self, query, key, value, dtype=jnp.float32, bias=None, axis=None,
                              broadcast_dropout=True, dropout_rng=None, dropout_rate=0,
                              deterministic=False, precision=None, mask=None):
        ...
        query_prime = self.kernel_feature_creator(query, self.projection_matrix, attention_dims_t,
                                                  batch_dims_t, precision, True)
        key_prime = self.kernel_feature_creator(key, self.projection_matrix, attention_dims_t,
                                                batch_dims_t, precision, False)
        ...
        if self.unidirectional:
            ...
            contract_query = tuple(range(len(batch_dims) + len(axis),
            ...
            if not self.renormalize_attention:
                ...
                contract_thick_all_ones = tuple(range(thick_all_ones.ndim - len(axis),
                                                      thick_all_ones.ndim))
                # Construct T = (K^{'})^{T} 1_L
                # k (bs, <non-attention dims>, num_heads, <attention dims>, channels)
                T = lax.dot_general(
                        key_prime,
                        thick_all_ones, ((contract_key, contract_thick_all_ones), (batch_dims_t, batch_dims_t)),
                        precision=precision)
