...
def preprocess_dataset(file_path, batch_size):
    ...
    ds = tf.data.experimental.make_csv_dataset(
        [file_path], batch_size, column_defaults=col_defaults, select_columns=sel_cols,
        field_delim=',', header=True, shuffle=False, num_epochs=1)
    ds = ds.unbatch()
    return ds
...
def get_imdb_dataset():
    ...
    data = tfds.load('imdb_reviews', data_dir='./tf_datasets')
...
def get_yelp_dataset():
    ...
    data = tfds.load('yelp_polarity_reviews', data_dir='./tf_datasets')
    train_raw = data['train']
    valid_raw = data['test']
    test_raw = data['test']
...
def get_agnews_dataset():
    ...
    data = tfds.load('ag_news_subset', data_dir='./tf_datasets')
    train_raw = data['train']
    valid_raw = data['test']
    test_raw = data['test']
...
def trim_num_data_entries(train, valid, test, num_data_entries=None):
    if num_data_entries is not None:
        return train.take(num_data_entries['train']), valid.take(num_data_entries['valid']), test.take(num_data_entries['test'])
    else:
        return train, valid, test
...
def get_tc_datasets(n_devices,
    ...
    train_dataset, val_dataset, test_dataset = trim_num_data_entries(
        train_dataset, val_dataset, test_dataset, num_data_entries=num_data_entries)

    tf.logging.info(f"Train/val/test dataset sizes: {len(train_dataset)}, {len(val_dataset)}, {len(test_dataset)}")
    ...
    if tokenizer == 'char':
        ...
        if fixed_vocab is None:
            ...
            for i, data in enumerate(train_dataset):
                ...
                if i % 1000 == 0:
                    tf.logging.info(f'Processed {i}')
            tf.logging.info(len(vocab_set))
            vocab_set = list(set(vocab_set))
            tf.logging.info(f'Finished processing vocab size={len(vocab_set)}')
        else:
